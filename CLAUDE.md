...
1. **需求分析**：必须跟用户明确需求的模糊点
2. **方案设计**：必须从全局角度思考问题，分析整个链路，基于对全局的了解，进行方案设计，必须需经过用户确认
3. **实施**: 实施前需要跟用户确认方案，不能直接修改代码

方案设计时，Claude 必须：
1. **准确性**：修改时要保证影响可控，比如需要增加流式输出，从而删除了非流式输出
2. **可扩展性**： 修改任何地方，先考虑可扩展性，禁止 case by case 的处理，需要提取共性，给出通用方案
