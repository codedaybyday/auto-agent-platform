 ...
 * - 防止路径穿越攻击
 * - 支持空间配额管理
 * - 沙盒根目录可配置（WORKSPACE_BASE_PATH 环境变量）
 */
...
export class WorkspaceSandbox {
  ...
  constructor(config: Partial<SandboxConfig> & { userId: string }) {
    const defaultBasePath = process.env.WORKSPACE_BASE_PATH ||
      join(homedir(), '.auto-agent', 'workspaces')
    ...
    log.info('WorkspaceSandbox', `Initialized sandbox for user: ${this.config.userId}`, {
  ...
  private ensureSandboxExists(): void {
    if (!existsSync(this.sandboxPath)) {
      ...
      log.info('WorkspaceSandbox', `Created sandbox directory: ${this.sandboxPath}`)
    }
  }
  ...
  getStats(): { size: number; fileCount: number; maxSize: number; maxFiles: number } {
    ...
    try {
      ...
      log.warn('WorkspaceSandbox', 'Failed to get stats', error)
    }
  ...
  cleanup(): void {
    try {
      if (existsSync(this.sandboxPath)) {
        ...
        log.info('WorkspaceSandbox', `Cleaned up sandbox: ${this.sandboxPath}`)
      }
    } catch (error) {
      log.error('WorkspaceSandbox', `Failed to cleanup sandbox: ${this.sandboxPath}`, error)
    }
  }
...
export class SandboxManager {
  ...
  constructor(basePath?: string) {
    this.basePath = basePath || process.env.WORKSPACE_BASE_PATH ||
      join(homedir(), '.auto-agent', 'workspaces')
    // 确保基础目录存在
  ...
  releaseSandbox(userId: string): void {
    ...
    if (sandbox) {
      ...
      log.info('SandboxManager', `Released sandbox for user: ${userId}`)
    }
  }
