...
export function setupAgentHandlers(mainWindow: BrowserWindow | null): void {
  ...
  ipcMain.handle('schedule:list', async () => {
    const { ws, generateId } = await import('../core/server-connection')
    if (!ws || ws.readyState !== 1) return { success: false, error: 'WebSocket 未连接' }
    return new Promise((resolve) => {
      const msgId = generateId()
      const handler = (raw: Buffer) => {
        try {
          const msg = JSON.parse(raw.toString())
          if (msg.type === 'schedule.list' && msg.messageId === msgId) {
      ...
      ws!.send(JSON.stringify({ type: 'schedule.list', messageId: msgId, timestamp: Date.now() }))
      setTimeout(() => { ws!.removeListener('message', handler); resolve({ success: false, error: '获取定时任务超时' }) }, 10000)
    })
  })
  ...
  ipcMain.handle('schedule:create', async (_event, data) => {
    ...
    return new Promise((resolve) => {
      ...
      const handler = (raw: Buffer) => {
        try {
          const msg = JSON.parse(raw.toString())
          if (msg.type === 'schedule.create' && msg.messageId === msgId) {
            ws!.removeListener('message', handler)
      ...
      ws!.on('message', handler)
      ws!.send(JSON.stringify({ type: 'schedule.create', messageId: msgId, timestamp: Date.now(), payload: data }))
      setTimeout(() => { ws!.removeListener('message', handler); resolve({ success: false, error: '创建超时' }) }, 10000)
    })
  })
  ...
  ipcMain.handle('schedule:update', async (_event, data) => {
    ...
    return new Promise((resolve) => {
      ...
      const handler = (raw: Buffer) => {
        try {
          const msg = JSON.parse(raw.toString())
          if (msg.type === 'schedule.update' && msg.messageId === msgId) {
            ws!.removeListener('message', handler)
      ...
      ws!.on('message', handler)
      ws!.send(JSON.stringify({ type: 'schedule.update', messageId: msgId, timestamp: Date.now(), payload: data }))
      setTimeout(() => { ws!.removeListener('message', handler); resolve({ success: false, error: '更新超时' }) }, 10000)
    })
  })
  ...
  ipcMain.handle('schedule:delete', async (_event, id: string) => {
    ...
    return new Promise((resolve) => {
      ...
      const handler = (raw: Buffer) => {
        try {
          const msg = JSON.parse(raw.toString())
          if (msg.type === 'schedule.delete' && msg.messageId === msgId) {
            ws!.removeListener('message', handler)
      ...
      ws!.on('message', handler)
      ws!.send(JSON.stringify({ type: 'schedule.delete', messageId: msgId, timestamp: Date.now(), payload: { id } }))
      setTimeout(() => { ws!.removeListener('message', handler); resolve({ success: false, error: '删除超时' }) }, 10000)
    })
  })
  ...
  ipcMain.handle('schedule:toggle', async (_event, id: string, enabled: boolean) => {
    ...
    return new Promise((resolve) => {
      ...
      const handler = (raw: Buffer) => {
        try {
          const msg = JSON.parse(raw.toString())
          if (msg.type === 'schedule.toggle' && msg.messageId === msgId) {
            ws!.removeListener('message', handler)
      ...
      ws!.on('message', handler)
      ws!.send(JSON.stringify({ type: 'schedule.toggle', messageId: msgId, timestamp: Date.now(), payload: { id, enabled } }))
      setTimeout(() => { ws!.removeListener('message', handler); resolve({ success: false, error: '操作超时' }) }, 10000)
    })
  })
}
